...
def processInputs(id, input_list):
    ...
    for i, input_file in enumerate(input_list, start=1):
        ...
        for patch_n, k in enumerate(range(noise_patch_per_img)):
...
def processInputsReport(id, input_list):
    ...
    # print(noise_patch_n_target, noise_patch_per_img, len(input_list))
    for i, input_file in enumerate(input_list, start=1):
        if extractDraw:
            # Caution! if there is no denoised_dir, silently killed.
            denoised_img=mrcfile.open(os.path.join(denoised_dir,input_file),permissive=True)
            denoised_img=denoised_img.data
            denoised_img=(denoised_img-np.min(denoised_img))/(np.max(denoised_img)-np.min(denoised_img))*255
        ...
        if patch_n != 1 and extractDraw:
            with mrcfile.new(draw_noise_path,overwrite=True) as draw_noise:
                draw_noise.set_data(denoised_img)
        file_end = time.time()
        eta_calc = (noise_patch_n_target - noise_patch_n) * (file_end-script_start)/noise_patch_n

        if noise_patch_n != noise_patch_n_target:
            print('# Random extract in representative thread : [{}/{}] {:.2%} || {:.4f} so far || eta : {:.4f}'.format(noise_patch_n, noise_patch_n_target, noise_patch_n/noise_patch_n_target,
...
def mainTaskReport(*input_list):
    processInputsReport(input_list[0], input_list[1])
