...
class Generator(torch.nn.Module):
    def __init__(self, channels):
        ...
        self.main_module = nn.Sequential(
            ...
            #nn.ConvTranspose2d(in_channels=, out_channels=128, kernel_size=4, stride=2, padding=1))
        ...
        for g_iter in range(self.generator_iters):
            ...
            for d_iter in range(self.critic_iter):
                ...
                if (images.size()[0] != self.batch_size):
                    print(images.size(), self.batch_size)
                    print("ERROR!")
                    continue
            ...
            if (g_iter) % SAVE_PER_TIMES == 0:
                ...
                if not os.path.exists('training_practice/'):
                    os.makedirs('training_practice/')
                ...
                utils.save_image(grid, 'training_practice/img_generatori_iter_{}.png'.format(str(g_iter).zfill(3)))
                #add by lihongjia 

                noise_out_0=mrcfile.new('training_practice/img_generatori_iter_{}_0.mrc'.format(str(g_iter)),overwrite=True)
                noise_out_1=mrcfile.new('training_practice/img_generatori_iter_{}_1.mrc'.format(str(g_iter)),overwrite=True)
                noise_out_2=mrcfile.new('training_practice/img_generatori_iter_{}_2.mrc'.format(str(g_iter)),overwrite=True)
                noise_out_3=mrcfile.new('training_practice/img_generatori_iter_{}_3.mrc'.format(str(g_iter)),overwrite=True)
                noise_out_4=mrcfile.new('training_practice/img_generatori_iter_{}_4.mrc'.format(str(g_iter)),overwrite=True)                
